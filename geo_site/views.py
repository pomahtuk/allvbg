  ...
  if request.method == 'GET':
    ...
    if form.is_valid():
      try:
    	...
    	error = True
  return render_to_response('site/firm_form.html', {'form': form, 'error':error}, context_instance=RequestContext(request))

def test_page(request):
	return render_to_response('site/test.html')
...
def ajax_firm_list(request):
	...
	return render_to_response('site/firm_ajax.html', {'firms': firms, 'lmt':lmt, 'pagecode':pagecode}, context_instance = RequestContext(request))
...
def thankyou(request):
	return render_to_response('site/contact_send.html')
...
def search(request):
    if 'q' in request.GET and request.GET['q']:
        ...
        try:
            ...
            firms = paginator.page(paginator.num_pages)		
        return render_to_response('site/main_templates/search.html', {'firms': firms, 'query': q, 'total': total}, context_instance=RequestContext(request))
    else:
        return render_to_response('site/main_templates/search.html', {'error': True}, context_instance=RequestContext(request))
...
def print_page(request, firm_id):
	...
	if not is_mobile(request):
		...
		if item.level == 0:
			return render_to_response('site/mobile/mobile_list.html', {'firm': item}, context_instance=RequestContext(request))
		elif item.level == 1:
...
def profile_view(request):
	return render_to_response('site/bootstrap/profile.html', {'user': request.user,}, context_instance=RequestContext(request))
...
def profile_edit_firm(request, firm_id):
	...
	if request.user.get_profile().editor_for == firm:
		if request.user.get_profile().paid_till >= datetime.now():
			firm=Firm.objects.get(pk=firm_id)
			return render_to_response('site/bootstrap/firm_edit.html', {'firm': firm}, context_instance=RequestContext(request))
		else:
...
def mobilesearch(request):
    if 'q' in request.GET and request.GET['q']:
        ...
        try:
            ...
            firms = paginator.page(paginator.num_pages)		
        return render_to_response('site/mobile/mobile_search.html', {'firms': firms, 'query': q, 'total':total}, context_instance=RequestContext(request))
    else:
        return render_to_response('site/mobile/mobile_search.html', {'error': True}, context_instance=RequestContext(request))
...
def sitemap(request):
	return render_to_response('site/sitemap.xml', {'zero': 0}, context_instance=RequestContext(request), mimetype="application/xml")

def rss(request):
	list=Firm.objects.order_by('-pub_date')[:20]
	return render_to_response('site/rss.xml', {'firms': list}, context_instance=RequestContext(request), mimetype="application/rss+xml")
