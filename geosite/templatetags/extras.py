...
from geosite.models import Event, Firm, Article
...
def pagination(firms, adjacent_pages=5):
    ...
    if 1 not in page_list:
        ...
        if 2 not in page_list:
            page_list.insert(1, '.')
    if firms.paginator.num_pages not in page_list:
...
def thumbnail(original_image_path, arg):
    ...
    basename, fmt = original_image_path.rsplit('.', 1)
    basename, name = basename.rsplit(os.path.sep, 1)

    miniature = name + '_' + str(max_size) + mode + '.' + fmt
    ...
    if not os.path.exists(miniature_filename) or os.path.getmtime(original_image_path) > os.path.getmtime(miniature_filename):
...
def month_cal(year=date.today().year, month=date.today().month):
    ...
    while day <= last_day_of_calendar:
        ...
        week.append(cal_day)
        if day.weekday() == 6:
            month_cal_arr.append(week)
            week = []
        i += 1
        day += timedelta(1)

    return {'calendar': month_cal_arr, 'headers': week_headers, 'extra':extra}
